...
//		//public float rightThumbAngle;
	...
	public static bool PollDepthFrame(SensorData sensorData, KinectManager_fruit.UserMapType userMapType,
	                                  bool bLimitedUsers, ICollection<int> alTrackedIndexes)
	{
		...
		if(sensorData.sensorInterface != null && !sensorData.isPlayModeEnabled)
		{
			...
			if(bNewFrame)
			{
				if(userMapType != KinectManager_fruit.UserMapType.RawUserDepth && sensorData.bodyIndexBuffer != null)
				{
				...
				if(sensorData.depthImageBuffer != null && sensorData.depthHistBuffer != null &&
				   userMapType == KinectManager_fruit.UserMapType.UserTexture)
				{
				...
				else if((userMapType == KinectManager_fruit.UserMapType.CutOutTexture || sensorData.sensorInterface.IsBackgroundRemovalActive()) && 
	...
	public static void RenderBodyIndexTexture(SensorData sensorData, KinectManager_fruit.UserMapType userMapType)
	{
		...
		if(sensorData.bodyIndexBufferReady)
		{
			...
			if(userMapType != KinectManager_fruit.UserMapType.UserTexture)
			{
				// buffer is released
				lock(sensorData.bodyIndexBufferLock)
				{
